...
pub async fn open_screen_selector(
  ...
  if let Some(existing_window) = app_handle.get_webview_window(&window_label) {
    ...
    match existing_window.show() {
      Ok(_) => match existing_window.set_focus() {
        Ok(_) => {
...
pub async fn process_screen_selection(
  ...
  if bounds.width <= 0 || bounds.height <= 0 {
    ...
    emit(OCR_RESPONSE, result).map_err(|e| format!("Failed to emit OCR response: {}", e))?;
    return Ok(());
  }
  ...
  let result: OcrResult =
    process_image_from_file(app_handle.clone(), pathbuf.to_str().unwrap().to_string())
      .await
      .map_err(|e| format!("Failed to process OCR: {}", e))?;

  // Delete temporary screenshot file
  std::fs::remove_file(pathbuf)
    .map_err(|e| format!("Failed to delete temporary screenshot file: {}", e))?;
  ...
  emit(OCR_RESPONSE, result.clone()).map_err(|e| format!("Failed to emit OCR response: {}", e))?;
