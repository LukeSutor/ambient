...
use crate::tasks::{
  models::*,
  service::TaskService,
  templates::{get_built_in_templates, get_categories, get_template_by_name},
};
use tauri::State;
...
#[tauri::command]
pub async fn get_task(db_state: State<'_, DbState>, task_id: i64) -> Result<TaskWithSteps, String> {
  TaskService::get_task_with_steps(&db_state, task_id)
}

/// Get all active tasks
#[tauri::command]
pub async fn get_active_tasks(db_state: State<'_, DbState>) -> Result<Vec<TaskWithSteps>, String> {
...
pub async fn get_available_frequencies() -> Result<Vec<(String, String)>, String> {
  let frequencies = vec![
    (
      TaskFrequency::OneTime.as_str(),
      TaskFrequency::OneTime.description(),
    ),
    (
      TaskFrequency::Daily.as_str(),
      TaskFrequency::Daily.description(),
    ),
    (
      TaskFrequency::Weekly.as_str(),
      TaskFrequency::Weekly.description(),
    ),
    (
      TaskFrequency::BiWeekly.as_str(),
      TaskFrequency::BiWeekly.description(),
    ),
    (
      TaskFrequency::Monthly.as_str(),
      TaskFrequency::Monthly.description(),
    ),
    (
      TaskFrequency::Quarterly.as_str(),
      TaskFrequency::Quarterly.description(),
    ),
    (
      TaskFrequency::Yearly.as_str(),
      TaskFrequency::Yearly.description(),
    ),
  ];
  Ok(frequencies)
}
...
/// Delete a task
#[tauri::command]
pub async fn delete_task(db_state: State<'_, DbState>, task_id: i64) -> Result<(), String> {
...
/// Get overdue tasks
#[tauri::command]
pub async fn get_overdue_tasks(db_state: State<'_, DbState>) -> Result<Vec<TaskWithSteps>, String> {
