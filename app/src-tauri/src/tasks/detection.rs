...
use crate::tasks::models::{ScreenContext, TaskDetectionResult, TaskStep};
use serde_json;
...
impl TaskDetectionService {
  ...
  pub fn build_detection_prompt(
    ...
    let template =
      get_prompt("TASK_DETECTION").expect("TASK_DETECTION prompt not found in prompts registry");
    ...
    template
      ...
      .replace(
        "{window_title}",
        screen_context.window_title.as_deref().unwrap_or("Unknown"),
      )
      .replace(
        "{screen_text}",
        &Self::truncate_screen_text(&screen_context.text, 2000),
      )
  }
  ...
  fn format_steps_for_prompt(steps: &[TaskStep]) -> String {
    ...
    steps
      ...
      .map(|step| {
        format!(
          "Step ID: {}\nStep {}: {}\nDescription: {}\nCurrent Status: {}\n",
          step.id, step.step_number, step.title, step.description, step.status
        )
      })
      .collect::<Vec<_>>()
      .join("\n---\n")
  ...
  fn validate_detection_result(result: &TaskDetectionResult) -> Result<(), String> {
    ...
    for step in &result.completed_steps {
      if step.reasoning.trim().is_empty() {
        return Err(format!(
          "Empty reasoning for completed step {}",
          step.step_id
        ));
      }
    }
