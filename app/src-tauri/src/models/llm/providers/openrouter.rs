...
impl LlmProvider for OpenRouterProvider {
    async fn generate(
        ...
        headers.insert(AUTHORIZATION, HeaderValue::from_str(&format!("Bearer {}", api_key)).map_err(|_| "Invalid API key header".to_string())?);
        ...
        if should_stream {
            ...
            while let Some(chunk) = stream.next().await {
                let Ok(chunk) = chunk.map_err(|e| format!("Error reading stream: {}", e)) else { break };
                ...
                for line in text.lines() {
                    ...
                    if let Ok(obj) = serde_json::from_str::<Value>(data) {
                        if let Some(choice) = obj.get("choices").and_then(|c| c.as_array()).and_then(|a| a.get(0)) {
                            if let Some(delta) = choice.get("delta").and_then(|d| d.as_object()) {
                                if let Some(piece) = delta.get("content").and_then(|c| c.as_str()) {
                                    full.push_str(piece);
                                    let _ = emit(CHAT_STREAM, ChatStreamEvent {
