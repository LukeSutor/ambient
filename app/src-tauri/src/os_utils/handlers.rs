...
pub static SCREEN_STATE_BUFFER: Lazy<Arc<Mutex<ScreenStateBuffer>>> = Lazy::new(|| {
    Arc::new(Mutex::new(ScreenStateBuffer::new()))
});
...
pub async fn handle_get_screen_diff(event: GetScreenDiffEvent, _app_handle: &AppHandle) {
    ...
    if let Some(prev_state) = &previous_state {
        for new_app in &new_data {
            ...
            if let Some(prev_app) = prev_state.data.iter().find(|&app| app.process_id == new_app.process_id) {
...
pub async fn capture_eval_data(app_handle: AppHandle) -> Result<String, String> {
    ...
    for new_app in &prev_state.data {
        if let Some(prev_app) = prev_prev_state.data.iter().find(|&app| app.process_id == new_app.process_id) {
            let prev_lines: HashSet<&String> = prev_app.text_content.iter().collect();
            let new_lines: Vec<String> = new_app.text_content.iter()
                .filter(|line| !prev_lines.contains(line))
                .cloned()
    ...
    let evals_dir = project_root.join("evals").join("task-detection").join("data");
    
    // Create directory if it doesn't exist
    fs::create_dir_all(&evals_dir)
        .map_err(|e| format!("Failed to create evals directory: {}", e))?;
    ...
    println!("[eval] Saved evaluation data to: {}", file_path_str);
