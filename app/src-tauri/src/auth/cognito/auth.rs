use crate::auth::types::{SignUpResult, SignInResult, CognitoUserInfo};
...
pub async fn sign_in(
    ...
    if let Some(auth_result) = auth_response.authentication_result {
        ...
        match store_cognito_auth(&sign_in_result) {
            Ok(()) => {
                println!("Authentication stored successfully");
                Ok(sign_in_result)
            }
            Err(e) => {
                eprintln!("Warning: Failed to store authentication: {}", e);
        ...
        match challenge_name.as_str() {
            ...
            "MFA_REQUIRED" => {
                Err("MFA required. Please contact administrator.".to_string())
            }
            _ => {
                Err(format!("Unhandled authentication challenge: {}", challenge_name))
            }
        }
    } else {
