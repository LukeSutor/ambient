    ...
    let app_data_dir = if cfg!(target_os = "windows") {
        std::env::var("APPDATA")
            ...
            .unwrap_or_else(|_| std::env::var("USERPROFILE")
                .map(|path| std::path::PathBuf::from(path).join("AppData").join("Roaming"))
                .unwrap_or_else(|_| std::path::PathBuf::from(".")))
            .join("local-computer-use")
    } else if cfg!(target_os = "macos") {
        std::env::var("HOME")
            .map(|path| std::path::PathBuf::from(path).join("Library").join("Application Support").join("local-computer-use"))
            .unwrap_or_else(|_| std::path::PathBuf::from(".").join("local-computer-use"))
    } else {
        // Linux
        std::env::var("HOME")
            .map(|path| std::path::PathBuf::from(path).join(".local").join("share").join("local-computer-use"))
            .unwrap_or_else(|_| std::path::PathBuf::from(".").join("local-computer-use"))
    };
