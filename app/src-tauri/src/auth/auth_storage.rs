...
// ============================================================================
// Legacy compatibility - Keep old functions working during migration
// ============================================================================

/// Legacy AuthToken type for backward compatibility
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct LegacyAuthToken {
...
/// Legacy store token (for backward compatibility)
pub fn store_token(token: &LegacyAuthToken) -> Result<(), Box<dyn std::error::Error>> {
    let entry = Entry::new(KEYRING_SERVICE, "oauth_tokens")?;
    let token_json = serde_json::to_string(token)?;
    entry.set_password(&token_json)?;
    Ok(())
}

/// Legacy retrieve token (for backward compatibility)
pub fn retrieve_token() -> Result<Option<LegacyAuthToken>, Box<dyn std::error::Error>> {
    let entry = Entry::new(KEYRING_SERVICE, "oauth_tokens")?;
    match entry.get_password() {
        Ok(token_json) => {
            let token: LegacyAuthToken = serde_json::from_str(&token_json)?;
            Ok(Some(token))
        }
