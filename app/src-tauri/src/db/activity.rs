...
pub fn insert_activity_summary(
  ...
  conn.execute(sql, rusqlite::params![summary, active_url, active_applications])
    .map_err(|e| format!("Failed to insert activity summary: {}", e))?;
  Ok(conn.last_insert_rowid())
}

/// Gets the most recent activity summary from the database.
pub fn get_latest_activity_summary(state: &State<DbState>) -> Result<Option<serde_json::Value>, String> {
  let conn_guard = state.0.lock().map_err(|_| "Failed to acquire DB lock".to_string())?;
  let conn = conn_guard.as_ref().ok_or("Database connection not available.".to_string())?;
...
#[tauri::command]
pub fn get_activity_summaries(state: State<DbState>, offset: u32, limit: u32) -> Result<serde_json::Value, String> {
  let conn_guard = state.0.lock().map_err(|_| "Failed to acquire DB lock".to_string())?;
  let conn = conn_guard.as_ref().ok_or("Database connection not available.".to_string())?;
