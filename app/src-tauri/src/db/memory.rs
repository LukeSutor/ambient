...
#[tauri::command]
pub fn insert_memory_entry(state: State<DbState>, memory_entry: MemoryEntry) -> Result<(), String> {
...
pub fn get_memory_entries(
  ...
  let rows = stmt
    .query_map(rusqlite::params![limit, offset], |row| {
      ...
      map.insert(
        "text".to_string(),
        serde_json::json!(row.get::<_, String>(3)?),
      );
      map.insert(
        "timestamp".to_string(),
  ...
  if let Some(q) = query {
      ...
      .map_err(|e| format!("Row fetch failed: {}", e))?
    {
      results.push(MemoryEntry {
        ...
        memory_type: row.get(2).map_err(|e| e.to_string())?,
        text: row.get(3).map_err(|e| e.to_string())?,
        timestamp: row.get(4).map_err(|e| e.to_string())?,
        embedding: Vec::new(),
        similarity: Some(row.get::<_, f64>(5).map_err(|e| e.to_string())?),
      });
    }
  } else {
    log::info!(
