...
pub fn insert_workflow(
  ...
  conn.execute(sql, rusqlite::params![name, description, url, steps_json, recording_start, recording_end, last_updated])
...
#[tauri::command]
pub fn get_workflows(state: State<DbState>, offset: u32, limit: u32) -> Result<serde_json::Value, String> {
  let conn_guard = state.0.lock().map_err(|_| "Failed to acquire DB lock".to_string())?;
  let conn = conn_guard.as_ref().ok_or("Database connection not available.".to_string())?;
  ...
  }).map_err(|e| format!("Query map failed: {}", e))?.collect::<Result<Vec<_>, _>>().map_err(|e| format!("Row processing failed: {}", e))?;
...
pub fn delete_workflow(state: State<DbState>, id: i64) -> Result<(), String> {
  ...
  let affected = conn.execute(sql, rusqlite::params![id]).map_err(|e| format!("Failed to delete workflow: {}", e))?;
  if affected == 0 { Err(format!("No workflow found with id {}", id)) } else { Ok(()) }
}
...
pub fn get_workflow_by_id(state: State<DbState>, id: i64) -> Result<serde_json::Value, String> {
  use rusqlite::params; use serde_json::json;
  let conn_guard = state.0.lock().map_err(|_| "Failed to acquire DB lock".to_string())?;
  let conn = conn_guard.as_ref().ok_or("Database connection not available.".to_string())?;
