...
#[tauri::command]
pub async fn call_main_sidecar(
  app_handle: tauri::AppHandle,
  image_path: String,
  prompt: String,
) -> Result<String, String> {
  println!(
    "[tauri] Received command to call main sidecar with image: {} and prompt: {}",
    image_path, prompt
  );
  ...
  let output = shell
    .command("main") // Or shell.sidecar("main") if configured
  ...
  if output.status.success() {
    ...
    println!(
      "[tauri] Sidecar execution failed. Status: {:?}, Stderr:\n{}",
...
pub async fn call_llama_sidecar(
  ...
  if output.status.success() {
    ...
    println!(
      "[tauri] Llama sidecar executed successfully. Output:\n{}",
      stdout
    );
    Ok(stdout)
  } else {
