...
C:\Users\Luke\Desktop\coding\local-computer-use\src-tauri\llm\server\build\bin\Release\qwen2vl-server.exe -m %MODEL_PATH% --mmproj %MMPROJ_PATH% --image %IMAGE_PATH% -p %PROMPT% -j %SCHEMA%